...
def deploy():
    ...
    cmds = [
        ...
        ["git", "checkout", "-B", "main"],
        ["git", "add", "."],
        ["git", "commit", "-m", "feat: enforce 100% advanced LLM agent for modifications, bump PWA to v4.5"],
    ...
    gh_cmds = [
        f"cd web; git init; git config user.name 'Eyal Andreson'; git config user.email 'eyal@example.com'; git checkout -B gh-pages; git add -A; git commit -m 'deploy: web dashboard to GitHub Pages (v4.5)'; git remote remove origin; git remote add origin {remote_url}; git push -u origin gh-pages --force"
    ]
